...
export default function AdminHeader({ adminView, onViewChange }: AdminHeaderProps) {
  return (
    <div className="bg-white border border-[#dfeafb] rounded-2xl p-4 shadow-sm">
      ...
      <p className="text-sm text-slate-500">Manage users, orders, inventory, and billing sales operations.</p>
      <div className="mt-4 inline-flex rounded-xl border border-[#d9e6fb] bg-[#f6faff] p-1">
        ...
        <button
          ...
          className={`px-3 py-1.5 text-sm rounded-lg ${adminView === 'sales' ? 'bg-white border border-[#d5e5ff] text-[#2d7ff9]' : 'text-slate-600'}`}
        >
          Sales
        </button>
      </div>
